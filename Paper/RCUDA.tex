...
Rather than have \Rfunc{.gpu} implicitly transfer data to the device,
we can explicitly transfer data from \R{} to the GPU with
\Rfunc{copyToDevice}.  By default, this allocates space on the device
...
once and copy different \R{} objects to that same space on the GPU at
different times.  This is slightly more efficient if we are applying
one or more kernels to different \R{} vectors or matrices with the
same number of elements.

THe \Rfunc{copyToDevice} function retrns an object that is an instance
...
will do this for us via the usual garbage collection.  When an \R{}
...
released.  This may mean we do not have enough memory on the GPU to
perform this new call to \Rfunc{.gpu}, eventhough we could use the
memory from the previous call.

In addition to the \Rfunc{cudaMalloc} function, there
are other functions to allocate memory on the GPU.
We can allocate 2-dimensional and 3-dimensional arrays
using \Rfunc{cudaMalloc}.
